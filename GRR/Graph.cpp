...
namespace iakov{
    ...
    const int Graph::isPointExists(std::string &name) {
    ...
    const bool Graph::hasArcs(int departmentPointIndex) {
    ...
    const bool Graph::isArcExists(int departmentPointName, int destinationPointName) {
    ...
    [[nodiscard]] bool Graph::isEmpty() const {
        if (numberOfPoints_ != 0) return false;
        return true;
    }

    const bool Graph::hasPoint(std::string &name) {
    ...
    const bool Graph::addPoint(std::string &name) {
        if (isPointExists(name) != -1) return false;
    ...
    const bool Graph::deletePoint(std::string &name) {
    ...
    void Graph::print() {
        ...
        for (int i = 0; i < numberOfPoints_; ++i) {
            ...
            std::copy(matrix_->operator()(i)->cbegin(), matrix_->operator()(i)->cend(),
                      std::ostream_iterator<int>(std::cout, " "));
            std::cout << "\n";
        }
    };

    const bool Graph::deleteArc(std::string &departmentPointName, std::string &destinationPointName) {
    ...
    const bool Graph::adArc(std::string &departmentPointName, std::string &destinationPointName, int mass) {
    ...
    const bool Graph::hasArcs(std::string &departmentPointName) {
        int departmentPointIndex = isPointExists(departmentPointName);
        return hasArcs(departmentPointIndex);
    };

    const bool Graph::isArcExists(std::string &departmentPointName, std::string &destinationPointName) {
    ...
    const bool Graph::wavePathFinderAlgorithm(std::string &departmentPointName, std::string &destinationPointName) {

        int departmentPointIndex = isPointExists(departmentPointName);
        int destinationPointIndex = isPointExists(destinationPointName);

        if (!hasArcs(departmentPointIndex) || !hasArcs(destinationPointIndex)) {
            std::cout << "Path can't be founded!\n";
            return false;
        }
    ...
    const bool Graph::dijkstraPathFinderAlgorithm(std::string &departmentPointName, std::string &destinationPointName) {
        ...
        while (!currentFixed->empty()) {
            ...
            for (int i = 0; i < matrix_->operator()(currentIndex)->size(); ++i) {
                if (matrix_->operator()(currentIndex, i) != 0 && !isPointFixed[i] &&
    ...
    const bool Graph::bellmanFordPathFinderAlgorithm(std::string &departmentPointName, std::string &destinationPointName) {
